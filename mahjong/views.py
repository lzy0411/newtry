...
def initialize_game(request):
    ...
    try:
        ...
        return _my_json_error_response("Not a valid game id",status=404)
...
def discard_tile_grab(request):
    ...
    try:
        ...
        return _my_json_error_response("No Valid Game ID",status=404)
...
def win_game(request):
    ...
    if game.winner is None and game.num_of_tiles_tba <=0:
        context = {"win_list": None,
                   "winner_id": None,
                   "winner": None,
                   "gameId":game.id,
